...
def _rawPageDataToTupleArray(raw, packing, pagesize, constflags, total_tuples):
    ...
    for i in range(0, n_pages):
        ...
        if constflags is not None:
            ...
            pv_steps = list(map(operator.mul, packing, pv_steps))
        else:
...
class Detail:
    ...
    def loadTopology(self, obj, primcount):
        ...
        _Assert((primcount == 0 or self.VertexMap)
                and type(self.VertexMap) == list,
    ...
    def loadJSON(self, file):
        ...
        self.loadTopology(file['topology'], file['primitivecount'])
...
def _ginfo(filename):
    try:
        with open(filename, 'r') as fp:
            _ginfoFP(fp, filename)
    except IOError:
        print('Unable to open', filename)
        return


def readGeoFile(filepath):
    ''' Read a Houdini geometry file and return a Detail object '''
...
def dumpGeoFileToPickle(geoFilePath, picklePath):
    ''' Read a Houdini geometry file and dump it to a pickle file '''
    import pickle
    d = readGeoFile(geoFilePath)
    with open(picklePath, 'wb') as pfp:
        pickle.dump(d, pfp)
...
def test():
    ...
    #     for f in sys.argv[1:]:
    #         _ginfo(f)
    # D:\Dev\HoudiniGeoIOSchema\testHip\default.geo
    _ginfo('D:/Dev/HoudiniGeoIOSchema/testHip/default.bgeo')

def testReadGeoFile():
    filepath = 'D:/Dev/HoudiniGeoIOSchema/testHip/default.bgeo'
    d = readGeoFile(filepath)
    print('Read geo file:', filepath)
    print('  Points:', d.pointCount())
    print('  Vertices:', d.vertexCount())
    print('  Primitives:', d.primitiveCount())
    dumpGeoFileToPickle(
        geoFilePath=filepath,
        picklePath='D:/Dev/HoudiniGeoIOSchema/output/default.pkl'
    )
    import pickle
    obj = pickle.load(open('D:/Dev/HoudiniGeoIOSchema/output/default.pkl', 'rb'))
    print('Pickle load:')
    print('  Points:', obj.pointCount())
    print('  Vertices:', obj.vertexCount())
    print('  Primitives:', obj.primitiveCount())
